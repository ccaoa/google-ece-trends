...
def raw_data_appending_prep(raw_gtrends_data_file: str) -> pd.DataFrame:
    ...
    transposed_sub_df = rc.transpose_df(subset_raw_data, first_col_as_new_col_names=True, old_cols_as_index=False,
...
def append_raw_data_from_files(raw_gtrends_data_files: list, suppress_prints: bool = False) -> dict:#pd.DataFrame:
    ...
    for app_spreadsheet in all_datasets_dict:
        ...
        if os.path.exists(app_spreadsheet) is False:
            ...
            appended_df = None  # rc.file_to_df(setup_file)  # ,raw_data_collection_sheet)
            init_setup = True
        else:
